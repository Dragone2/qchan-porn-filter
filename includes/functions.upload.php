...
function setup_dir() {
	...
	if(!file_exists(ABSPATH.'/'.UPLOAD_DIR))
		mkdir(ABSPATH.'/'.UPLOAD_DIR) || exit('Impossibile scrivere nella cartella di upload.');
	...
	if(!file_exists(ABSPATH.'/'.THUMB_DIR))
		mkdir(ABSPATH.'/'.THUMB_DIR) || exit('Impossibile scrivere nella cartella di anteprima.');
			...
			if($files['size'][$key] > get_size_limit()) {
				...
				if($duplicate = is_duplicate($temp)) {
					...
					if(!isset($result['status']) || !$result['status'] == 'failed') {
						...
						include_once("class.ImageFilter.php");
						$Obj = new ImageFilter;
